"""DEITA quality scorer — single-step generate with softmax expectation.

Uses a specialized model (hkust-nlp/deita-quality-scorer) that generates
a single token and computes the expected quality score from the softmax
...
QUALITY_TEMPLATE = (
    "You are a helpful assistant. Please identify the quality score "
    "of the Response corresponding to the Question. \n"
    "#Question#:\n{instruction}\n#Response#:\n{output} \n##Quality: "
)
...
class DeitaQualityScorer(BaseScorer):
    """Score response quality on a 1-6 scale via DEITA single-step generation."""
    ...
    def _ensure_model(self) -> None:
        ...
        logger.info("DeitaQualityScorer: loading %s", self.model_path)
        ...
        logger.info("DeitaQualityScorer: ready on %s", self._device)
    ...
    def _score_batch(self, items: list[dict]) -> list[float]:
        ...
        for item in items:
            ...
            if input_text:
                instr = f"{instr}\n{input_text}"
            prompts.append(QUALITY_TEMPLATE.format(instruction=instr, output=output))
    ...
    async def score(self, data: list[dict], output_dir: str | Path) -> list[dict]:
        self._ensure_model()
        raw: list[float] = []
        for i in tqdm(range(0, len(data), self.batch_size), desc="DeitaQualityScorer"):
